...
def new_vector_formation(all_peaks):
  final_vector = []
  x_max = 0
  y_max = 0
  x_min = 639
  y_min = 479
  for l in range(len(all_peaks)):
    vector_of_joints = []
    if len(all_peaks[l]) == 0:
      final_vector.append(vector_of_joints)
      continue
    else:
      for k in range(len(all_peaks[l])):
        joint = all_peaks[l][k]
        vector_of_joints.append([joint[0],joint[1]])
        if joint[0]>x_max:
          x_max = joint[0]
        if joint[0]<x_min:
          x_min = joint[0]
        ...
        if joint[1]<y_min:
          y_min = joint[1]
      final_vector.append(vector_of_joints)
  return final_vector, x_max, y_max, x_min, y_min
  
def dynamic_vector_formation(person):
  final_vector = []
...
def scale(width, height,final_vector):
  for m in range(len(final_vector)):  
    for c in range(len(final_vector[m])):
      final_vector[m][c][1] = final_vector[m][c][1]*height/width
  return final_vector

def new_fill_miss_parts(final_vector,prev_frame):
  for i in range(len(final_vector)):
    if len(final_vector[i])==0 and len(prev_frame[i])!=0:
      for j in range(len(prev_frame[i])):
        if prev_frame[i][j]!=[None,None]:
          dx = prev_frame[1][0][0] - prev_frame[i][j][0]
          dy = prev_frame[1][0][1] - prev_frame[i][j][1]
          x = final_vector[1][0][0] - dx
          y = final_vector[1][0][0] - dy
          final_vector[i].append([x,y])
        else:
          final_vector[i].append([None,None])
  return final_vector
